...
$db=Baglan("mysql10.000webhost.com","a9896778_heychat","a9896778_heychat","1234+asd");
$kul=KullaniciGet();
if($query = $db->prepare("SELECT Icerik.icerik,Login.kuLadi FROM Icerik inner join Login on Icerik.gNiD=Login.id WHERE (gNiD = ".$_POST["gID"]." and aLiD=".$kul[0]["id"].") or (gNiD = ".$kul[0]["id"]." and aLiD=".$_POST["gID"].") order by Icerik.id asc")){
	$query->execute();
		$result = $query->fetchAll(PDO::FETCH_ASSOC);
		return $result;
}
